...
func (p *Parser) Parse(ctx context.Context, lexer *Lexer) error {
	return p.parse(ctx, 5, 5870, lexer)
}
...
func lookaheadRule(ctx context.Context, lexer *Lexer, next, rule int32, s *session) (sym int32, err error) {
	...
	case 3745:
		var ok bool
		if ok, err = lookahead(ctx, lexer, next, 0, 5864, s); ok {
	...
	case 3746:
		var ok bool
		if ok, err = lookahead(ctx, lexer, next, 1, 5865, s); ok {
	...
	case 3747:
		var ok bool
		if ok, err = lookahead(ctx, lexer, next, 4, 5868, s); ok {
	...
	case 3748:
		var ok bool
		if ok, err = lookahead(ctx, lexer, next, 3, 5867, s); ok {
			sym = 830 /* lookahead_StartOfFunctionType */
		} else {
	...
	case 3749:
		var ok bool
		if ok, err = lookahead(ctx, lexer, next, 2, 5866, s); ok {
...
func (p *Parser) applyRule(ctx context.Context, rule int32, lhs *stackEntry, rhs []stackEntry, lexer *Lexer, s *session) (err error) {
	...
	case 3748:
		...
		if ok, err = AtStartOfFunctionType(ctx, lexer, p.next.symbol, s); ok {
			lhs.sym.symbol = 830 /* lookahead_StartOfFunctionType */
		} else {
