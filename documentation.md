...
Textmapper is designed in such a way that all these approaches can be mixed in the same grammar. Nonterminals and rules that are explicitly mapped to AST classes are excluded from the "deriving" mechanism.

AST handling features can be turned on in the language header by adding a *"with ast"* suffix.

	language sql (in java) with ast;

Appropriate data structures in the target language are then generated for declared and derived AST types. For grammar rules that are mapped to the AST, Textmapper deduces semantic actions, which instantiate AST nodes.

## AST Definition

An AST model can be defined in the grammar in a separate ```:: ast``` section. This way its definition is usually shorter than in target languages, and is portable across them. The actual mapping from this high-level definition onto the target language constructs is made in the language-specific templates. An AST model is formed of the following types:

* primitives (int, bool and string)
* enumerations
* classes and interfaces (types of AST nodes)
* lists of nodes
* raw types (any valid type in the target language, e.g. ```float```)

AST classes consist of fields, may have a superclass, and implement any number of interfaces. All nodes stored in fields are contained within the class and considered to be its parts. There are no cross-references in the AST generated by Textmapper.
	...
	class Block {
	    StatementList statementList[0..1];  # optional statement list
	}
	
	class StatementList {
	    Statement statements[1..n];         # non-empty list of statements
	}
	
	class IfStatement : Statement {
	    Expression condition[1];
	    Block ifStatement[1];
	    Block elseStatement;                # [0..1] is the default cardinality
	}

Interfaces may contain fields as well. Enumerations are defined as finite sets of literals.

	:: ast
	interface AstNamedEntity {
	    string name;
	}
	...
	class ClassQualifiedReference {
	    string nameParts[1..n];
	}
	
	class AstClass : AstNamedEntity {
	    Visibility visibility;
	    ClassQualifiedReference superClass;
	}
	
To use a raw type, enclose it into parentheses. Fields of raw types cannot have cardinalities.
...
Defining an AST model in a grammar is wordy and explicit, so for many simple nonterminals it is worth using automatically derived types. For example, *Visibility* declaration could have been skipped and deduced from the grammar nonterminal. Then we can refer to it using the ```parser.NonterminalName``` notation for a derived type.
...
In order to get a separately defined AST instantiated and returned from the parser, we must specify which nonterminals and grammar rules correspond to which AST types. When a nonterminal is mapped to a class, all its rules start producing instances of this class by default. We can prefix any symbol on the right-hand side with ```fieldName=``` to store its value in the field. In the following example, the value of the ID token is stored in the name field (we assume that ID is declared to be of the string type).
...
Return types can be specified on individual actions, which in turn allows us to use interfaces and abstract classes as nonterminal types. Optional square brackets in front of a rule can contain various rule-related modifiers, including the ```returns AstTypeName``` clause. 

	:: ast
	abstract class ConstExpression : Expression {
	}
	
	class IntegerLiteral : ConstExpression {
	    int val;
	}
	
	class StringLiteral : ConstExpression {
	    string val;
	}
...
A parenthesized clause can be mapped to a field. In this case it is considered as a separate rule producing its own data. Textmapper automatically deduces its type from the type of the field, but we can specify one by adding the ```[returns AstTypeName]``` modifier after the opening parenthesis. The content of the parentheses is then mapped to their type. Such mappings allow us to produce more than one node from a grammar rule.
	...
	complexRule returns Container ::=
	      ...
	      # equivalent to new Container(new ChildEx($ID));
	    | 'container' '(' child=([returns ChildEx] 'childEx' name=ID ')'
	;

If we don't need an intermediate node for the rule and just want to pass the wrapped value upwards, we can map its symbol to ```this```. Mapping to ```this``` must be the only mapping in the rule.
...
```+=``` can also be used to handle last/first elements of the list separately.

	:: parser
	class Method {
	    string name;
	    Parameter params[0..n];
	}
