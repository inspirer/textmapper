...
import org.textway.lapg.test.cases.bootstrap.nla.NlaTestParser.ParseException;

public class NlaTestTree<T> {

	private final TextSource source;
	private final T root;
	private final List<NlaTestProblem> errors;

	public NlaTestTree(TextSource source, T root, List<NlaTestProblem> errors) {
		this.source = source;
		this.root = root;
		this.errors = errors;
	}
	...
	public List<NlaTestProblem> getErrors() {
		return errors;
	}
	...
	public static NlaTestTree<Object> parse(TextSource source) {
		...
		try {
			NlaTestLexer lexer = new NlaTestLexer(source.getStream(), reporter);
			lexer.setLine(source.getInitialLine());

			NlaTestParser parser = new NlaTestParser(reporter);
			Object result = parser.parse(lexer);

			return new NlaTestTree<Object>(source, result, list);
	...
	public static class NlaTestProblem extends Exception {
