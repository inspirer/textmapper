...
import org.textway.lapg.test.oldparser.LapgLexer.LapgSymbol;
import org.textway.lapg.test.oldparser.LapgLexer.Lexems;
...
public class LapgParser implements LapgLexer.ErrorReporter {
	...
	private final Map<String,CSymbol> symCash = new HashMap<String,CSymbol>();
	private final List<String> errors = new ArrayList<String>();

	private final List<CSymbol> symbols = new ArrayList<CSymbol>();
	private final List<CRule> rules = new ArrayList<CRule>();
	private final List<CPrio> prios = new ArrayList<CPrio>();
	private final List<CInputDef> inputs = new ArrayList<CInputDef>();
	...
	private void addGrammarDirective( String id, List<CSymbol> list, int line ) {
		if( id.equals("left") ) {
			prios.add(new CPrio(Prio.LEFT, list,inputId,line));
		} else if( id.equals("right") ) {
			prios.add(new CPrio(Prio.RIGHT, list,inputId,line));
		} else if( id.equals("nonassoc") ) {
			prios.add(new CPrio(Prio.NONASSOC, list,inputId,line));
