...
import { NodeType, Listener } from './listener'
import { Parser, SyntaxError, ErrorMsg } from './parser'
import { Selector } from './selector';
import { Lexer } from './lexer';
import { Tree, Node } from './tree'

/**
* Parses the given content string  into an AST {@link Tree}.
*
...
*/
export function createTree(content: string): { tree?: Tree, err?: Error } {
  const parseFunc = (p: Parser, l: Lexer) : { err? : SyntaxError } => {
    return p.parse(l);
  };
  return create(content, parseFunc);
}

function create(content: string, parseFunc: (p: Parser, s: Lexer) => { err? : SyntaxError }): { tree?: Tree, err?: Error } {
  const builder = new TreeBuilder(content);
  const eventListener: Listener = (t: NodeType, offset: number, endOffset: number) => {
    builder.addNode(t, offset, endOffset);
  };
  const l = new Lexer(content);
  const p = new Parser(eventListener);

  let res = parseFunc(p, l);
  if (res.err) {
...
class TreeBuilder {
  ...
  addNode(t: NodeType, offset: number, endOffset: number) {
...
class DefaultTree implements Tree {
  ...
  constructor(content: string) {
    ...
    this._root = new DefaultNode(this, 0, 0, content.length);
  }
