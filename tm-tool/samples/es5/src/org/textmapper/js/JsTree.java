...
import com.test.JsLexer.ErrorReporter;
import com.test.JsParser.ParseException;
...
public class JsTree<T> {
	...
	public static JsTree<Object> parse(TextSource source) {
		final List<JsProblem> list = new ArrayList<JsProblem>();
		...
		try {
			...
			return new JsTree<Object>(source, result, list);
		} catch (ParseException ex) {
			/* not parsed */
		} catch (IOException ex) {
			list.add(new JsProblem(KIND_FATAL, "I/O problem: " + ex.getMessage(), 0, 0, 0, ex));
		}
		return new JsTree<Object>(source, null, list);
	}
