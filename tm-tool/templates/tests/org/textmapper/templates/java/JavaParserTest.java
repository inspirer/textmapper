...
public class JavaParserTest {
	...
	private AstNode parse(final String text) throws IOException {
		...
		JavaParser p = new JavaParser(reporter) {
			...
			protected void shift() throws IOException {
				recorder.add(tmSymbolNames[tmNext.symbol], text.substring(tmNext.offset, tmNext.endoffset),
						tmNext.offset, tmNext.endoffset, false);
				super.shift();
			}
		};
	...
	private static class TreeRecorder {
		...
		public void add(String symbol, String text, int offset, int endoffset, boolean simpleRule) {
			...
			if (node.children != null && node.children.get(0).symbol.equals(symbol) && !doNotMerge.contains(node
					.symbol)) {
				// flatten lists
